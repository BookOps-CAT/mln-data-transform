...
class TestTeacherSetBuilder:
    def test_create_validate_teacher_set(self, set_test_data, mock_responses, caplog):
        builder = TeacherSetBuilder(file="data/foo_bar.csv")
        teacher_set = builder.create_teacher_set(**set_test_data)
        builder.validate_set(teacher_set)
        assert len(teacher_set.parts) == 2
        assert len(caplog.records) == 7
        # should be 2 x the number of components + 3
        assert teacher_set.enhanced is None
        assert len(teacher_set.parts) == 2
        assert [i.msg for i in caplog.records] == [
            ...
            "ISBN 9780987654321: retrieving full bib record (OCLC number: ocn123456789).",
        ]
